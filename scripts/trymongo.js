...
const testWithAsync = () => {
  ...
  async.waterfall([
    ...
    (connection, next) => {
      db = connection;
      db.collection('employeer').insertOne({ id: 1, name: 'D. Async' }, next);
    },
    (insertResult, next) => {
      console.log('InsertResult: ', insertResult.insertedId);
